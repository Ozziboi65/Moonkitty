...
public class Tracer extends Feature {
    ...
    public void init() {
        ...
        HudRenderCallback.EVENT
                .register((drawContext, counter) -> tickDelta = counter.getTickProgress(true));

        WorldRenderEvents.BEFORE_DEBUG_RENDER.register(context -> {
            if (!isEnabled() || client.player == null || client.world == null)
                return;

            Vec3d origin = client.player.getLerpedPos(tickDelta);

            for (AbstractClientPlayerEntity player : client.world.getPlayers()) {
                if (player == client.player)
                    continue;
                if (renderPlayer)
                    GizmoDrawing.line(origin, player.getLerpedPos(tickDelta), 0xFFFF0000);
            }
        });
    }
}
